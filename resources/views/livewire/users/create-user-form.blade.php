{{-- <form wire:submit.prevent="saveUser" method="POST">
    <x-form.input name="name" label="name" wire:model="name" required autofocus />

    <div class="flex flex-col gap-0 md:flex-row md:gap-2.5">
        <div class="w-full md:w-1/2">
            <x-form.input required name="designation" label="designation" wire:model="designation" />
        </div>
        <div class="w-full md:w-1/2">
            <x-form.input required name="email" label="email" wire:model="email" />
        </div>
    </div>

    <div class="flex flex-col gap-0 md:flex-row md:gap-2.5">
        <div class="w-full md:w-1/2">
            <x-form.input required name="mobile" label="mobile" wire:model="mobile" minlength='10' maxlength='10' />
        </div>
        <div class="w-full md:w-1/2">
            <x-form.select required name="role_id" label="role" wire:model.live="role_id">
...
<form wire:submit.prevent="saveUser" method="POST">
    <x-form.input name="invitation.name" label="name" wire:model="invitation.name" required autofocus />

    <div class="flex flex-col gap-0 md:flex-row md:gap-2.5">
        <div class="w-full md:w-1/2">
            <x-form.input required name="invitation.designation" label="designation" wire:model.defer="invitation.designation" />
        </div>
        <div class="w-full md:w-1/2">
            <x-form.input required name="invitation.email" label="email" wire:model.defer="invitation.email" />
        </div>
    </div>

    <div class="flex flex-col gap-0 md:flex-row md:gap-2.5">
        <div class="w-full md:w-1/2">
            <x-form.input required name="invitation.mobile" label="mobile" wire:model.defer="invitation.mobile" minlength='10' maxlength='10' />
        </div>
        <div class="w-full md:w-1/2">
            <x-form.select required name="invitation.role_id" label="role" wire:model.live="invitation.role_id">
                <option value="">Select</option>
                @foreach ($roles as $role)
                <option value="{{ $role['id'] }}">{{ $role['name'] }}</option>
                @endforeach
