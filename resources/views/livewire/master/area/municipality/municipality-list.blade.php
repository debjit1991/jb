        ...
        <div class="flex flex-col md:flex-row md:items-center gap-2.5 w-full md:w-1/2">
            <div class="relative rounded-md shadow-sm">
                ...
                <input type="text" name="search" id="search" wire:model.live="searchItem"
                    class="focus:ring-indigo-500 focus:border-indigo-500 municipality w-full px-10 sm:text-sm border-gray-300 rounded-md"
                    placeholder="Search Municipality Name" autocomplete="off">
            ...
            <span class="flex text-sm font-medium text-gray-500">Total Available: {{ $municipalities->total() }}</span>
        </div>
    </div>
        ...
        <div class="divide-y-1 divide-gray-200">
                    ...
                    <div
                        ...
                        <div class="sm:hidden flex w-full">
                            ...
                            <div class="flex gap-2 w-1/5">
                                ...
                                <button type="button" onclick="confirmDeleteMunicipality('{{ $municipality->id }}')"
                        ...
                        <div class="hidden sm:flex flex-col justify-center text-gray-500">
                            <span class="text-sm font-bold mt-1">{{ $municipality->ref_code }}</span>
                        </div>
                        <div class="hidden sm:flex flex-col justify-center text-gray-500">
                            <span class="text-sm font-bold mt-1">{{ $municipality->lg_code }}</span>
                        </div>
                        <div class="hidden sm:flex flex-col justify-center text-gray-500">
                            <span class="text-sm font-bold mt-1">{{ $municipality->district->name }}</span>
                        </div>
            ...
            @empty
                <div class="flex p-4 items-center justify-center bg-white">
                    {{-- <x-empty-state message="No Municipalities Found" /> --}}
                </div>
            @endforelse
        </div>
    </div>
...
@once
    @push('scripts')
        <script>
            // Confirm box for Deleting Municipality.
            const confirmDeleteMunicipality = (id) => {
                Swal.fire({
                    title: 'Are you sure?',
                    text: "Confirm if you want to delete the Municipality",
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        @this.deleteMunicipality(id).then(
                            (response) => {
                                if (response.status == 200)
                                    ...
                                    Swal.fire({
                                        title: 'Error in Deleting Municipality',
                                        text: "Municipality can not be deleted",
                                        icon: 'error',
                                    });
                            },
                        );
